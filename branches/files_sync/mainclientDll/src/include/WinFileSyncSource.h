...
#ifndef INCL_WINFILES_SYNC_SOURCE
#define INCL_WINFILES_SYNC_SOURCE
...
/**
 * This class extends the FileSyncSource class, it's specialized for files items. 
 * The sync direction is fixed on "one-way-from-server", so the method getAllItemList()
 * symply does nothing. 
 * "filesConfig::folderPath" and "dir" have the same meaning, so they are set at
 * the same value in the constructor.
 * Configuration paramenters that need to be saved inside the registry are stored in the
 * WindowsSyncSourceConfig& filesConfig object, owned by OutlookConfig.
 */
class WinFileSyncSource : public FileSyncSource
{
...
public:
     ...
     * value, which is the shell folder for files
