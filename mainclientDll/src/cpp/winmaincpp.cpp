...
void GetOutlookVersionString2(LPTSTR* lppszPath, LPTSTR* lppszVer, LPTSTR* lppszLang)
{
	...
	TCHAR pszaOutlookQualifiedComponents[][MAX_PATH] = {
		TEXT("{BC174BAD-2F53-4855-A1D5-0D575C19B1EA}"), // O11_CATEGORY_GUID_CORE_OFFICE (retail)
		TEXT("{BC174BAD-2F53-4855-A1D5-1D575C19B1EA}"), // O11_CATEGORY_GUID_CORE_OFFICE (debug)
		TEXT("{24AAE126-0911-478F-A019-07B875EB9996}"), // O12_CATEGORY_GUID_CORE_OFFICE (retail)
		TEXT("{1E77DE88-BCAB-4C37-B9E5-073AF52DFD7A}")  // O14_CATEGORY_GUID_CORE_OFFICE (retail)
	};
	int nOutlookQualifiedComponents = 4;
...
HRESULT GetOutlookVersionString(LPTSTR* ppszVer, BOOL* pf64Bit)
{
    ...
    int nOutlookQualifiedComponents = 3;
...
int initializeClient(bool isScheduled, bool justRead) {
    ...
    LPTSTR lpszVer = NULL;
    BOOL bits = FALSE;
	GetOutlookVersionString(&lpszVer, &bits);

    if (lpszVer) {
        LOG.info(">>> Outlook version = '%ls'", lpszVer);
    }
    LOG.info(">>> is %s bit", bits? "64":"32");


    lpszVer = NULL;
    LPTSTR lpszPath = NULL;
    LPTSTR lpszLang = NULL;
    GetOutlookVersionString2(&lpszPath, &lpszVer, &lpszLang);
    if (lpszVer) {
        LOG.info(">>> Outlook path2 = '%ls'", lpszPath);
    }
    if (lpszVer) {
        LOG.info(">>> Outlook version2 = '%ls'", lpszVer);
    }
    if (lpszLang) {
        LOG.info(">>> Outlook lang2 = '%ls'", lpszLang);
    }
